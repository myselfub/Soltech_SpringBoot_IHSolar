...
String.prototype.zf = function (len) { return "0".toString(len - this.length) + this; };
...
(function(window, $) {
	...
	function Workspace() {
		...
		this.images.check.src = getContextPath() + "/resources/js/biznexus/images/check.png";
		...
		let resetEffectByMouse = function(param) {
			...
			for (let shape of shapeList.shapes) {
				...
				if (!shape.effectByMouseList) continue;
		
				let shapeNames = shapeList.shapeNames;
		
				for (let effect of shape.effectByMouseList) {
					if (!effect || effect.event !== param) continue;
					if (!effect.panel || effect.panel[0] === '$') continue;
		
					let target = shapeNames[effect.panel];
					if (target) {
		...
		let doEffectByMouse = function(src, flag) {
			...
			// 새 코드
			if (!src.effectByMouseList) return;

			let shapeList = workspace_self.shapeManager.getShapeList();
			let shapeNames = shapeList.shapeNames;
		
			for (let effect of src.effectByMouseList) {
				if (!effect || !effect.panel) continue;
				...
				let shape = shapeNames[effect.panel];
		...
		this.onClick = function(e) {
			...
			let doEffectByToggleImage = function(shape) {
				...
				shape.fill.image.src = 
					getContextPath() + "/resources/js/biznexus/images/" + shape.effectByToggleImage.states[seq].state + ".png";
			}
			...
			for(let i = nLen; i >= 0; i--) {
				...
				if(e.offsetX >= shape.left  && e.offsetX <= shape.left + shape.width &&
					...
					if(workspace_self.edit == false) {
						...
						if (shape.effectByMouseList !== undefined) { doEffectByMouse(shape, true); break; } 
						else if (shape.effectByMouse !== undefined) { doEffectByMouse(shape, true); break; }
					}
				}
			}
		...
		this.onMouseMove = function(e) {
			...
			if(workspace_self.edit == true) {
				...
				if(shapeOrCursor.shape != null) {
					...
					let shape = shapeOrCursor.shape;

					if (shape.effectByMouseList !== undefined) {
						let hasOverEvent = shape.effectByMouseList.some(effect => effect.event === 'over');
						if (hasOverEvent) {
							if (hoverShape !== shape) {
								resetEffectByMouse('over');
								doEffectByMouse(shape, true);
								hoverShape = shape;
							}
						}
					}
					else if (shape.effectByMouse !== undefined) {
						if (shape.effectByMouse.event === 'over') {
							if (hoverShape !== shape) {
								resetEffectByMouse('over');
								doEffectByMouse(shape, true);
								hoverShape = shape;
		...
		this.updateShapes = function(s) {
			...
			let setEffectByMouse = function(shape, s) {
				...
				// 새 코드
				if (Array.isArray(s.effectByMouseList) && s.effectByMouseList.length > 0) {
						shape.effectByMouseList = s.effectByMouseList.map(e => Object.assign({}, e));
				}
				else if (s.effectByMouse !== undefined) {
					shape.effectByMouseList = [ Object.assign({}, s.effectByMouse) ];
				}
				else {
					delete shape.effectByMouseList;
					delete shape.effectByMouse;
				}
			}
			let setEffectByToggleImage = function(shape, s) {
				if(Object.keys(s.effectByToggleImage).length === 0) {
					...
					shape.fill.image = new Image();
					shape.fill.image.onload = function() { shape.bDraw = true; workspace_self.refresh(); };
					shape.fill.image.src = getContextPath() + "/resources/js/biznexus/images/" + s.effectByToggleImage.states[0].state + ".png";
				}
			}
			...
			let setEffectBySwitch = function(shape, s) {
				if(s.effectBySwitch.control === undefined) { 
					...
					shape.fill.image = new Image();
					shape.fill.image.onload = function() { shape.bDraw = true; workspace_self.refresh(); };
					shape.fill.image.src = getContextPath() + "/resources/js/biznexus/images/" + s.effectBySwitch.states[0] + ".png";
				}
			}
			...
			// 새 코드
			if (s.effectByMouse !== undefined || s.effectByMouseList !== undefined) fn = setEffectByMouse;
		...
		function CanvasManager() {
			...
			this.onSize = function() {
				let width = window.innerWidth;	//  self.canvas.offsetWidth;
...
//						if(shape.bDraw == false) continue;
			...
			let testEffectBySwitch = function(tagValues, shape) {
				let changeState = function(nVal) {
					...
					let src = getContextPath() + "/resources/js/biznexus/images/" + shape.effectBySwitch.states[nVal] + ".png"; 
					shape.fill.image = new Image();
				...
				if(shape.effectBySwitch !== undefined) {
					if(shape.effectBySwitch.status == '') {
						...
						if(tag !== undefined) {
							...
							if(nVal != shape.effectBySwitch.val ) changeState(nVal);
						}
					}
				}
			}
		...
		function Shape() {
			...
			this.drawChart = function(context) {
				...
				let spacing = 10;
				if ('barSpacing' in self.effectByChart) {
					let tmp = Number(self.effectByChart.barSpacing);
					if (!isNaN(tmp)) spacing = tmp;
				}
				let barWidthRatio = 1.0;
				if ('barWidthRatio' in self.effectByChart) {
					let tmp = Number(self.effectByChart.barWidthRatio);
					if (!isNaN(tmp)) barWidthRatio = tmp;
				}
				let roundType = "full";
				let roundR = 0;
				if ('round' in self.effectByChart) {
					if (typeof self.effectByChart.round === 'object') {
						if ('type' in self.effectByChart.round) roundType = self.effectByChart.round.type;
						if ('r' in self.effectByChart.round) roundR = Number(self.effectByChart.round.r);
					}
				}
				
				let minValRight = 0, maxValRight = 100;
				let hasRightAxis = self.effectByChart.options.some(opt => opt.axis === 'right');
				...
				let recalcPlotArea = function() {
					...
					if (hasRightAxis && self.effectByChart.labelYRight.length > 0) {
						let labels = self.effectByChart.labelYRight.split(",");
						for (let label of labels) {
							let w = context.measureText(label + self.effectByChart.unitRight).width + 10;
							right -= w;
							width -= w;
							break;
						}
						let labelsRight = self.effectByChart.labelYRight.split(",");
						minValRight = Number.MAX_VALUE; maxValRight = Number.MIN_VALUE;
						for (let label of labelsRight) {
							minValRight = Math.min(minValRight, Number(label));
							maxValRight = Math.max(maxValRight, Number(label));
						}
					}
				...
				let drawAxisX = function() {
					...
					gapX = (width - ((labels.length - 1) * spacing)) / labels.length; 
				...
				let drawAxisYRight = function() {
					...
					for (let i = labels.length - 1; i >= 0; i--) {
						if (i != labels.length - 1) {
							let label = labels[i] + self.effectByChart.unitRight;
							let x = right + 8;
							context.fillText(label, x, y);
						}
						y += gap;
				...
				let drawAxisXY = function() {
					...
					context.lineTo(right + 5, bottom);
					context.moveTo(left - 5, top + 10);
					context.lineTo(left - 5, bottom);
					if (hasRightAxis) {
						context.moveTo(right + 5, top + 10);
						context.lineTo(right + 5, bottom);
					}
					context.stroke();
				}
				...
				let drawBarChart = function(option, index) {
					...
					for(let val of values) {
						if(val > minVal) {
							...
							let gap = (gapX / nSeries) * barWidthRatio;
							let barX = x + index * (gapX / nSeries);
							context.fillStyle = Common.getFillStyle(context, option.color, barX, top, gap, h);
							context.fillRect(barX, y, gap, h);
									
							if (option.displayValue == true) {
								let offset = (gap - context.measureText(val.toFixed(1)).width) / 2;
								context.fillText(val.toFixed(1), barX + offset, y - 5);
							}
						}
						x += gapX + spacing;
					}	
				...
				let drawLineChart = function(option) {
					let values = getValues(option);
					let scaleMin = (option.axis === "right") ? minValRight : minVal;
					let scaleMax = (option.axis === "right") ? maxValRight : maxVal;
					...
					let x = left;
					for (let val of values) {
						let h = ((val - scaleMin) / (scaleMax - scaleMin)) * (bottom - top);
						let y = Math.max(bottom - h, top);
						if (x == left)
				...
				for(let i = 0; i < self.effectByChart.options.length; i++) {
					...
					switch(option.type) {
						...
						case "3" : drawStepChart(option, i); break;				// 스텝 CHART
					}
				}
				drawAxisXY();
			}
			...
			this.drawGauge = function(context) {
				...
				let drawValue = function() {
					...
					context.fillStyle = Common.getFillStyle(context, self.font.color, self.left, self.top, self.width, self.height);
					context.fillText(value, rx - m1.width / 2, ry - 0.5 * m.width + 50);
			...
			this.fillImage = function(context) {
				try {
					if(typeof(this.fill.image) != 'undefined') {
						...
						// 새 코드
						if (self.bOver === true && (
								self.effectByMouseList !== undefined || 
								self.effectByMouse !== undefined)) {
							context.shadowColor = "Lime";
							context.shadowBlur = 5;
						}
					...
					self.fill.image.src = getContextPath() + "/resources/js/biznexus/images/noimage80.png";
				}
			}
		...
		function ShapeList() {
			...
			let showTooltip = function(src) {
				...
				// 새 코드
				let effects = src.effectByMouseList || (src.effectByMouse ? [src.effectByMouse] : []);
				for (let effect of effects) {
					if (!effect || effect.panel[0] !== '$') continue;
			
					for (let i = shapes.length - 1; i >= 0; i--) {
						let shape = shapes[i];
						if (shape.name[0] !== '$') break;
						if (shape.name !== effect.panel) continue;
		...
		function ShapeManager() {
			let PreLoader = function() {
				...
				let noImage = getContextPath() + "/resources/js/biznexus/images/noimage80.png"
			...
			let onEffectByMouse = function(xmlNode, shape) {
				...
				// 새 코드
				if (!shape.effectByMouseList) shape.effectByMouseList = [];
				let effect = { event: '', panel: '', position: 'none' };

				for (let childNode of xmlNode.childNodes) {
					if (childNode.firstChild == null) continue;
					switch (childNode.nodeName) {
						case "event": effect.event = childNode.firstChild.data; break;
						case "panel": effect.panel = childNode.firstChild.data; break;
						case "position": effect.position = childNode.firstChild.data; break;
					}
				}
			
				shape.effectByMouseList.push(effect);
			}
			...
			let onEffectBySwitch = function(xmlNode, shape) {
				...
				let src = getContextPath() + "/resources/js/biznexus/images/" + shape.effectBySwitch.states[0] + ".png";
			...
			let onEffectByToggleImage = function(xmlNode, shape, preLoader) {
				...
				let src = getContextPath() + "/resources/js/biznexus/images/" + shape.effectByToggleImage.states[0].state + ".png";
				shape.fill.image = preLoader.loadImage(src);
			...
			let onEffectByChartSeries = function(xmlNode, shape) {
				...
				for (let childNode of xmlNode.childNodes) {
					...
					case "axis": option.axis = childNode.firstChild.data; break;
					}
				}
			...
			let onEffectByChart = function(xmlNode, shape) {
				shape.effectByChart = { options:[], labelX:'', labelY:'', unit:'', labelYRight:'', unitRight:'', axisX:'hour' };
				for (let childNode of xmlNode.childNodes) {
					if(childNode.firstChild == null) continue;
					if(childNode.nodeName.indexOf('series') >= 0) {
						...
						case "label-y-right": shape.effectByChart.labelYRight = childNode.firstChild.data; break;
						case "unit": shape.effectByChart.unit = childNode.firstChild.data; break;
						case "unitRight": shape.effectByChart.unitRight = childNode.firstChild.data; break;
						case "barSpacing": shape.effectByChart.barSpacing = parseFloat(childNode.firstChild.data); break;
						case "barWidthRatio": shape.effectByChart.barWidthRatio = parseFloat(childNode.firstChild.data); break;
						case "round":
							shape.effectByChart.round = {};
							for (let roundChild of childNode.childNodes) {
								if (roundChild.firstChild == null) continue;
								switch(roundChild.nodeName) {
									case "type":
										shape.effectByChart.round.type = roundChild.firstChild.data;
										break;
									case "r":
										shape.effectByChart.round.r = parseFloat(roundChild.firstChild.data);
										break;
								}
							}
							break;
			...
			*/
			...
			let writeXmlShapes = function(doc, root) {
				...
				let setEffectByMouse = function(effectByMouseNode, shape) {
					// 기존 코드
					...
					// 새 코드
					if (!effectByMouseNode) {
						effectByMouseNode = doc.createElement("effectByMouse");
					}
					if (shape.effectByMouseList !== undefined) {
						for (let effect of shape.effectByMouseList) {
							let node = doc.createElement("effectByMouse");
							appendAndSetNode(doc, node, 'event', effect.event);
							appendAndSetNode(doc, node, 'panel', effect.panel);
							appendAndSetNode(doc, node, 'position', effect.position);
							effectByMouseNode.appendChild(node);
						}
					} else if (shape.effectByMouse !== undefined) {
						let node = doc.createElement("effectByMouse");
						appendAndSetNode(doc, node, 'event', shape.effectByMouse.event);
						appendAndSetNode(doc, node, 'panel', shape.effectByMouse.panel);
						appendAndSetNode(doc, node, 'position', shape.effectByMouse.position);
						effectByMouseNode.appendChild(node);
					}
				}
				...
				*/
				...
				let setNode = function(doc, node, shape) {
					...
					// 새 코드
					if (shape.effectByMouseList !== undefined) {
						for (let i = 0; i < shape.effectByMouseList.length; i++) {
							createNode(doc, node, 'effectByMouse').set(setEffectByMouse, {effectByMouse: shape.effectByMouseList[i]});
						}
					} else if (shape.effectByMouse !== undefined) {
						createNode(doc, node, 'effectByMouse').set(setEffectByMouse, shape);
					}
						...
						if(workspace_self.edit == true) {
							...
							for(let shape of shapeList.shapes) {
								...
								if (shape.effectByMouseList !== undefined) {
									...
									for (let effect of shape.effectByMouseList) {
										if (!effect || !effect.panel) continue;
										if (effect.panel[0] === '$') continue;
								
										let panelShape = shapeNames[effect.panel];
										if (panelShape) {
											panelShape.visible = false;
										}
									}
								} else if (shape.effectByMouse !== undefined) {
									if (shape.effectByMouse.panel[0] !== '$') {
										let shapeNames = workspace_self.shapeManager.getShapeList().shapeNames;
										let panelShape = shapeNames[shape.effectByMouse.panel];
										if (panelShape) {
											panelShape.visible = false;
										}
									}
